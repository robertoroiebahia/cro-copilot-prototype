  ...
  const filteredExperiments = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(e =>
        e.experiment_id.toLowerCase().includes(query) ||
    ...
    return filtered;
  }, [experiments, selectedStatus, searchQuery]);

  const stats = useMemo(() => {
    return {
      total: experiments.length,
      not_started: experiments.filter(e => e.status === 'not_started').length,
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl font-black text-brand-black">Experiments</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-2xl font-black text-gray-600">{stats.not_started}</div>
            <div className="text-xs text-brand-text-secondary uppercase font-bold">Not Started</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-2xl font-black text-purple-600">{stats.running}</div>
            <div className="text-xs text-brand-text-secondary uppercase font-bold">Running</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-2xl font-black text-green-600">{stats.completed}</div>
            <div className="text-xs text-brand-text-secondary uppercase font-bold">Completed</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-2xl font-black text-yellow-600">{stats.paused}</div>
            <div className="text-xs text-brand-text-secondary uppercase font-bold">Paused</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-2xl font-black text-red-600">{stats.cancelled}</div>
            <div className="text-xs text-brand-text-secondary uppercase font-bold">Cancelled</div>
          </div>
        </div>
...
function ExperimentCard({ experiment, onUpdate }: { experiment: Experiment; onUpdate: () => void }) {
  ...
  const updateStatus = async (newStatus: Experiment['status']) => {
    try {
      ...
      if (newStatus === 'running' && !experiment.started_at) {
        updates.started_at = new Date().toISOString();
      } else if (newStatus === 'completed' && !experiment.completed_at) {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-xs font-black text-brand-text-secondary">{experiment.experiment_id}</span>
              <span className={`px-2 py-1 ${config.color} text-xs font-bold rounded border uppercase`}>
                {config.icon} {experiment.status.replace('_', ' ')}
              </span>
              {experiment.test_spec?.platform && (
            ...
            <h3 className="text-xl font-black text-brand-black mb-2">{experiment.test_spec?.name || 'Untitled Experiment'}</h3>
            {experiment.test_spec?.description && (
              <p className="text-brand-text-secondary">{experiment.test_spec.description}</p>
            )}
          </div>
        </div>
